...
# cap = cv2.imread("con1.jpg")

# Empezamos
while True:
    # Leemos los fotogramas
    #frame = cv2.resize(cap, (0, 0), None, 0.25, 0.25)
    ret, frame = cap.read()
    # Leemos imagen
    ...
    if resrostros.detections is not None:
        ...
        for rostro in resrostros.detections:
            ...
            if gen == 'Man':
                ...
                emociones_get = ['angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral']
                emociones_set = ['enojado', 'disgustado', 'miedoso', 'feliz', 'triste', 'sorprendido', 'neutral']

                for i in range(len(emociones_get)):
                    if emociones_get[i] == emociones:
                        emociones = emociones_set[i]

                race_get = ['asian', 'indian', 'black', 'white', 'middle eastern', 'latino hispanic']
                race_set = ['asiatico', 'indio', 'negro', 'blanco', 'oriente medio', 'latino']

                for i in range(len(race_get)):
                    if race_get[i] == race:
                        race = race_set[i]
                ...
                emociones_get = ['angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral']
                emociones_set = ['enojada', 'disgustada', 'miedosa', 'feliz', 'triste', 'sorprendida', 'neutral']
                ...
                race_get = ['asian', 'indian', 'black', 'white', 'middle eastern', 'latino hispanic']
                race_set = ['asiatica', 'india', 'negra', 'blanca', 'oriente medio', 'sorprendida', 'latina']
